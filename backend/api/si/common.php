...
function si_latest_summary(string $module, int $userId, int $siteId, string $tabKey): array
{
    ...
    return si_summary_from_run($run, $module, $tabKey);
}

function si_summary_from_run(array $run, string $module, string $tabKey): array
{
    $runId = (int)$run['id'];
...
function si_summary_history(string $module, int $userId, int $siteId, string $tabKey, int $limit = 10): array
{
    $conn = si_db();
    ...
    return array_map(
        static function (array $run) use ($module, $tabKey): array {
            return si_summary_from_run($run, $module, $tabKey);
        },
        $runs
    );
